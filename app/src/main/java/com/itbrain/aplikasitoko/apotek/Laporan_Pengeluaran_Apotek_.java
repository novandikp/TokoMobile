...
public class Laporan_Pengeluaran_Apotek_ extends AppCompatActivity {
    ...
    public void getbeli(String cari) {
        arrayList.clear();
        RecyclerView recyclerView = (RecyclerView) findViewById(R.id.rcLapKel) ;
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setHasFixedSize(true);
        RecyclerView.Adapter adapter = new Adapter_Laporan_Dua_Apotek(this,arrayList) ;
        recyclerView.setAdapter(adapter);
        String q = ModulApotek.selectwhere("qbelidetail")+ ModulApotek.sLike("fakturorderbeli",cari)+" OR "+ ModulApotek.sLike("supplier",cari)+" AND "+ ModulApotek.sBetween("tglorderbeli", ModulApotek.getText(v,R.id.tgl1), ModulApotek.getText(v,R.id.tgl2));
        double total=0;
        Cursor c = db.sq(q) ;
        while(c.moveToNext()){
            double jumlah = ModulApotek.strToDouble(ModulApotek.getString(c,"jumlahbeli"))* ModulApotek.strToDouble(ModulApotek.getString(c,"hargabeli"));
            total=total+jumlah;
            String campur = "PEMBELIAN"+"__"+ ModulApotek.getString(c,"fakturorderbeli") + "__" + ModulApotek.getString(c,"tglorderbeli")+ "__" + ModulApotek.getString(c,"supplier")+ "__" + ModulApotek.getString(c,"barang")+ "__" + ModulApotek.getString(c,"jumlahbeli")+ "__" + ModulApotek.getString(c,"satuanbeli")+ "__" + ModulApotek.getString(c,"hargabeli")+ "__" + ModulApotek.getString(c,"batchnumber");

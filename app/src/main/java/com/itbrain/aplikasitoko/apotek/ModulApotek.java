...
public class ModulApotek {
    ...
    public ModulApotek(SharedPreferences sharedPreferences) {
    ...
    public static String getexpired(int hari){
        Calendar calendar = Calendar.getInstance();
        int year=calendar.get(Calendar.YEAR);
        int month=calendar.get(Calendar.MONTH);
        int day=calendar.get(Calendar.DAY_OF_MONTH);
        return setDatePickerNormal(year,month+1,day+hari);
    }
    public static String getExpiredEnd(int hari){
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_MONTH, hari);
        int year=calendar.get(Calendar.YEAR);
        int month=calendar.get(Calendar.MONTH);
        int day=calendar.get(Calendar.DAY_OF_MONTH);
        return setDatePickerNormal(year,month+1,day);
    }
    ...
    public static String setDatePickerNormal(int year , int month, int day) {
        String bln,thn,hri ;
        if(month < 10){
            bln = "0"+ ModulApotek.intToStr(month) ;
        } else {
            bln = ModulApotek.intToStr(month) ;
        }

        if(day < 10){
            hri = "0"+ ModulApotek.intToStr(day) ;
        } else {
            hri = ModulApotek.intToStr(day) ;
        }

        return hri+"/"+bln+"/"+ ModulApotek.intToStr(year);
    }
    public static String setDatePickerNormalNew(int year , int month, int day) {
        String bln, thn, hri;
        if (month < 10) {
            bln = "0" + ModulApotek.intToStr(month);
        } else {
        ...
        return ModulApotek.intToStr(year)+ "-" + bln + "-" + hri ;
    }
...
//            String c = b.replace("-",".") ;
    ...
    public static String unchangeComma(String s){
        try {
            String a = s.replace(".",",") ;
//            String c = b.replace("-",".") ;
            return a;
    ...
    public static Boolean copyFile(String pIn, String pOut, String name){
        ...
        try{
            ...
            Log.d("file error copy", e.getMessage());
            return false ;
        }
    }
...
//    }
//
    public static String sederhana(double value){
        DecimalFormat df = new DecimalFormat("#");
        df.setMaximumFractionDigits(0);
        return ModulApotek.numberFormat(df.format(value)) ;
    }

    public static String sederhana (String value){
        double hasil = ModulApotek.strToDouble(value) ;
        DecimalFormat df = new DecimalFormat("#");
        df.setMaximumFractionDigits(0);
        return ModulApotek.numberFormat(df.format(hasil)) ;
    }
...
//    }
    ...
    public static String slimit(String tabel,String keyCari, String pencarian){
        ...
        if(TextUtils.isEmpty(pencarian)){
            hasil = ModulApotek.select(tabel) +" LIMIT 30"  ;
        } else {
            hasil = ModulApotek.selectwhere(tabel) + ModulApotek.sLike(keyCari,pencarian) ;
        }
        return hasil ;
    }

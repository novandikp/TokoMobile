...
public class ModulTokoSepatu {
    SharedPreferences sharedPreferences;
    SharedPreferences.Editor editor;
    static String db = "dbsepatuplus";
    static String dirBackup = "dbsepatuplus";
    static int version = 1;


    ModulTokoSepatu(SharedPreferences sharedPreferences) {
    ...
    public static String setDatePickerNormal(int year , int month, int day) {
        String bln,thn,hri ;
        if(month < 10){
            bln = "0"+ ModulTokoSepatu.intToStr(month) ;
        } else {
            bln = ModulTokoSepatu.intToStr(month) ;
        }

        if(day < 10){
            hri = "0"+ ModulTokoSepatu.intToStr(day) ;
        } else {
            hri = ModulTokoSepatu.intToStr(day) ;
        }

        return hri+"/"+bln+"/"+ModulTokoSepatu.intToStr(year);
    }
...
//            String c = b.replace("-",".") ;
    ...
    public static String hilange(double value){
        ...
        return ModulTokoSepatu.numberFormat(df.format(value)) ;
    }
    ...
    public static String hilange(String value){
        double hasil = ModulTokoSepatu.strToDouble(value) ;
        DecimalFormat df = new DecimalFormat("#");
        df.setMaximumFractionDigits(8);
        return ModulTokoSepatu.numberFormat(df.format(hasil)) ;
    }
...
//            hasil=hasil.replaceAll("-.","-");
    ...
    public static String slimit(String tabel,String keyCari, String pencarian){
        ...
        if(TextUtils.isEmpty(pencarian)){
            hasil = ModulTokoSepatu.select(tabel) +" LIMIT 30"  ;
        } else {
            hasil = ModulTokoSepatu.selectwhere(tabel) +ModulTokoSepatu.sLike(keyCari,pencarian) ;
        }
        return hasil ;
    }

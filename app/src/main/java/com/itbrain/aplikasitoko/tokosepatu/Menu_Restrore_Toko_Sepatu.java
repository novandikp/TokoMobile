...
import com.itbrain.aplikasitoko.tokosepatu.Menu_Restrore_Toko_Sepatu;

import java.io.File;
import java.util.ArrayList;

public class Menu_Restrore_Toko_Sepatu  extends AppCompatActivity {
    ListView listView;
    View v;
    String dirOut, dirIn;
    ...
    protected void onResume() {
        super.onResume();
        dirOut = getPref.getString("dirRestore", getExternalFilesDir(null).getAbsolutePath()+"/"+ "/Backup/");
        ModulTokoSepatu.setText(v, R.id.viewPathRestore, dirOut);
    }
    ...
    private void readFile() {
        ...
        ArrayAdapter arrayAdapter = new com.itbrain.aplikasitoko.tokosepatu.AdapterBackup(this,R.layout.list_item_restore_kain,R.id.wadah,arrayList) ;
        ...
        if(isi.length > 0){
            for(int i = 0 ; i < isi.length ; i++){
                ...
                try {
                    String hasil = nama.substring(DatabaseTokoSepatu.nama.length()) ;
                    if(nama.substring(0,(nama.length()-16)).equals(DatabaseTokoSepatu.nama)){
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        if(ModulTokoSepatu.deleteFile(dirIn+DatabaseTokoSepatu.nama)){
                            if(ModulTokoSepatu.copyFile(dirOut,dirIn,db)){
                                File a = new File(dirIn+db) ;
                                File b = new File(dirIn+DatabaseTokoSepatu.nama) ;
                                a.renameTo(b) ;
                                Toast.makeText(Menu_Restrore_Toko_Sepatu.this, "Reset Berhasil, Aplikasi terestart", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(Menu_Restrore_Toko_Sepatu.this, "Restore Gagal", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(Menu_Restrore_Toko_Sepatu.this, "Restore Gagal", Toast.LENGTH_SHORT).show();
                        }
    ...
    public void hapus(View v){
        ...
        alertDialog.setMessage("Anda Yakin Ingin Menghapus")
                .setPositiveButton("Ya", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        if(ModulTokoSepatu.deleteFile(path+db)) {
                            Toast.makeText(Menu_Restrore_Toko_Sepatu.this, "Hapus Backup Berhasil", Toast.LENGTH_SHORT).show();
                            readFile();
                        }else{
                            Toast.makeText(Menu_Restrore_Toko_Sepatu.this, "Hapus Backup Gagal", Toast.LENGTH_SHORT).show();
                        }
                    }
                })

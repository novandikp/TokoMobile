...
public class TextWatcherDiskon_Sepatu implements TextWatcher {
    ...
    public TextWatcherDiskon_Sepatu(EditText et, String total, EditText baru, Locale locale, int numDecimals, Context c) {
...
//        et.setKeyListener(DigitsKeyListener.getInstance("0123456789.,"));
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        double harga= ModulTokoSepatu.strToDouble(total);
        if(ModulTokoSepatu.strToDouble(ModulTokoSepatu.unNumberFormat(et.getText().toString()))>harga){
            ModulTokoSepatu.showToast(c,"Potongan harga lebih besar daripada total pembelian");
            et.setText("0");
        }else{
            harga= harga-ModulTokoSepatu.strToDouble(ModulTokoSepatu.unNumberFormat(et.getText().toString()));
            baru.setText(ModulTokoSepatu.setCurrency(harga));
        }

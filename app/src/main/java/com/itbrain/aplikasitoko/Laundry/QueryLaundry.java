...
public class QueryLaundry {
    ...
    public static String sBetween(String key, String v1, String v2){
        return key+" BETWEEN "+ ModulLaundry.quote(v1)+" AND "+ ModulLaundry.quote(v2) ;
    }
    ...
    public static String deleteFrom(String table,String key,String values){
        return "DELETE FROM "+table+" WHERE "+key+"="+values;
    }
        ...
        if(pecah.length > 1){
            ...
            if((p.length+1) != pecah.length){
                ...
                for (i = 0; i < p.length; i++) {
                    fix += pecah[i] + ModulLaundry.quote(p[i]) ;
                }
                fix += pecah[i] ;

...
public class Restore_Data_Klinik_ extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView = (ListView) findViewById(R.id.lvRestore) ;
        try {
    ...
    private void readFile() {
        ...
        if(isi.length > 0){
            for(int i = 0 ; i < isi.length ; i++){
                ...
                try {
                    String hasil = nama.substring(DatabaseKlinik.nama.length()) ;
                    if(nama.substring(0,(nama.length()-16)).equals(DatabaseKlinik.nama)){
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        if(ModulKlinik.deleteFile(dirIn+DatabaseKlinik.nama)){
                            if(ModulKlinik.copyFile(dirOut,dirIn,db)){
                                File a = new File(dirIn+db) ;
                                File b = new File(dirIn+DatabaseKlinik.nama) ;
                                a.renameTo(b) ;
                                ModulKlinik.showToast(Restore_Data_Klinik_.this,"Reset Berhasil, Aplikasi terestart");
                                finishAffinity();

                                System.exit(0);

                            } else {
                                ModulKlinik.showToast(Restore_Data_Klinik_.this,"Restore Gagal");
                            }
                        } else {
                            ModulKlinik.showToast(Restore_Data_Klinik_.this,"Restore Gagal");
                        }
    ...
    public void hapus(View v){
        ...
        String path = dirOut ;
        if(ModulKlinik.deleteFile(path+db)){

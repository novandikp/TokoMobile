...
//        }
    ...
    private void getPendapatanTeknisi() {
        arrayList.clear();
        total=0;
        ...
        while(c.moveToNext()){
            ...
            if (!ModulKlinik.getString(c,"iddokter").equals("1")){
                double pendapatan=100-ModulKlinik.strToDouble(ModulKlinik.getString(c,"bagihasil"));
                pendapatan=pendapatan/100;
                harga=ModulKlinik.doubleToStr(ModulKlinik.strToDouble(harga)*pendapatan);
            }


            total+=ModulKlinik.strToDouble(harga);

            String campur = ModulKlinik.getString(c,"idperiksa")+"__"+ModulKlinik.getString(c,"fakturperiksa") + "__" + ModulKlinik.getString(c,"tglperiksa")+ "__" + ModulKlinik.getString(c,"pasien")+ "__" + ModulKlinik.getString(c,"jasa")+"__" + ModulKlinik.getString(c,"keterangan")+"__" + harga+"__dokter";
    ...
    private String whereTeknisi() {
        Spinner sTeknisi = findViewById(R.id.sTeknisi);
        String idteknisi;
        if (sTeknisi.getSelectedItemPosition()==0){

            String b = ModulKlinik.selectwhere("view_detailperiksa")+ " flagperiksa=2 AND "+ModulKlinik.sBetween("tglperiksa",ModulKlinik.getText(v,R.id.tglawal),ModulKlinik.getText(v,R.id.tglakhir))+ModulKlinik.sOrderASC("fakturperiksa");
            return b;
        }else{
            idteknisi = ModulKlinik.intToStr(sTeknisi.getSelectedItemPosition()+1);
            String q=ModulKlinik.selectwhere("view_detailperiksa")+" iddokter="+idteknisi+" AND flagperiksa=2 AND "+ModulKlinik.sBetween("tglperiksa",ModulKlinik.getText(v,R.id.tglawal),ModulKlinik.getText(v,R.id.tglakhir));
            return q;
        }
...
//            }
    ...
    public void export(View view) {
        Intent i= new Intent(Laporan_Bagi_Hasil_Klinik_.this,MenuExportExcelKlinik.class);
        i.putExtra("type","pendapatan2");
        startActivity(i);
    }
    }

class AdapterBagiHasil extends RecyclerView.Adapter<AdapterBagiHasil.ViewHolder> {
    ...
    public AdapterBagiHasil(Context a, ArrayList<String> kota) {
        this.data = kota;
        c = a;
    }

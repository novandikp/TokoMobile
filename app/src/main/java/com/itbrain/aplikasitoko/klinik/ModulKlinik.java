...
public class ModulKlinik {
    ...
    public ModulKlinik(SharedPreferences sharedPreferences) {
    ...
    public static String setDatePickerNormal(int year , int month, int day) {
        String bln,thn,hri ;
        if(month < 10){
            bln = "0"+ ModulKlinik.intToStr(month) ;
        } else {
            bln = ModulKlinik.intToStr(month) ;
        }

        if(day < 10){
            hri = "0"+ ModulKlinik.intToStr(day) ;
        } else {
            hri = ModulKlinik.intToStr(day) ;
        }

        return hri+"/"+bln+"/"+ModulKlinik.intToStr(year);
    }
    public static String setTimePickerNormal(int hour , int minute) {
        String jam,thn,mnt ;
        if(hour < 10){
            jam = "0"+ ModulKlinik.intToStr(hour) ;
        } else {
            jam = ModulKlinik.intToStr(hour) ;
        }

        if(minute < 10){
            mnt = "0"+ ModulKlinik.intToStr(minute) ;
        } else {
            mnt = ModulKlinik.intToStr(minute) ;
        }
...
//            String c = b.replace("-",".") ;
     ...
     public static String getUmur(String date){
        String date2 = ModulKlinik.getDate("yyyyMMdd");
        int tahun2 = ModulKlinik.strToInt(date2.substring(0,4));
        int tahun1 = ModulKlinik.strToInt(date.substring(0,4));
        if(tahun2-tahun1>3){
            return ModulKlinik.intToStr(tahun2-tahun1)+" tahun";
        }else{
            int bulan = tahun2-tahun1;
            bulan*=12;
            int bulan1 = ModulKlinik.strToInt(date2.substring(4,6))+bulan;
            int bulan2 = ModulKlinik.strToInt(date.substring(4,6));
            return ModulKlinik.intToStr(bulan1-bulan2)+" bulan";
        }
     }
    ...
    public static String hilange(double value){
        ...
        return ModulKlinik.numberFormat(df.format(value)) ;
    }
    ...
    public static String hilange(String value){
        double hasil = ModulKlinik.strToDouble(value) ;
        DecimalFormat df = new DecimalFormat("#");
        df.setMaximumFractionDigits(8);
        return ModulKlinik.numberFormat(df.format(hasil)) ;
    }
...
//    }
    ...
    public static String slimit(String tabel,String keyCari, String pencarian){
        ...
        if(TextUtils.isEmpty(pencarian)){
            hasil = ModulKlinik.select(tabel) +" LIMIT 30"  ;
        } else {
            hasil = ModulKlinik.selectwhere(tabel) +ModulKlinik.sLike(keyCari,pencarian) ;
        }
        return hasil ;
    }

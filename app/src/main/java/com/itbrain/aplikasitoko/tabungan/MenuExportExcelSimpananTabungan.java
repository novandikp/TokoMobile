...
public class MenuExportExcelSimpananTabungan extends AppCompatActivity {
    String tab;
    View v;
    DatabaseTabungan db;
    String path;
...
//        ModulTabungan.btnBack("Export Excel",getSupportActionBar());
    ...
    public void export(View view) throws IOException, WriteException {
        ...
        nama = getText(R.string.laporan_simpanan).toString();
        exSimpanan();

    }


    private void exSimpanan() throws IOException, WriteException{
        ...
        if(c.getCount() > 0){
            ...
            while(c.moveToNext()){
                int col = 0 ;
                Cursor sumSaldo=db.sq(QueryTabungan.selectwhere("tbltransaksi")+QueryTabungan.sWhere("idsimpanan",ModulTabungan.getString(c,"idsimpanan")));
                sumSaldo.moveToLast();
                String namaSimpanan = ModulTabungan.getString(c,"simpanan") ;
                String jenis = ModulTabungan.getString(c,"jenis") ;
                String bunga = ModulTabungan.getString(c,"bunga") ;
                String anggota = ModulTabungan.getString(c,"namaanggota") ;
                String jumlahSaldo = ModulTabungan.removeE2(ModulTabungan.getDouble(sumSaldo,"saldo"));

                addLabel(sheet,col++, row, ModulTabungan.intToStr(no));
                addLabel(sheet,col++, row, namaSimpanan);
                addLabel(sheet,col++, row, jenis);
                addLabel(sheet,col++, row, bunga);
                addLabel(sheet,col++, row, anggota);
                addLabel(sheet,col++, row, jumlahSaldo);
    ...
    private void exTransaksi() throws IOException, WriteException{
        ...
        Cursor c = db.sq(QueryTabungan.select("tbltransaksi")) ;
        if(c.getCount() > 0){
            int no = 1 ;
            while(c.moveToNext()){
                int col = 0 ;
                Cursor cSimpanan=db.sq(QueryTabungan.selectwhere("qsimpanan")+QueryTabungan.sWhere("idsimpanan",ModulTabungan.getString(c,"idsimpanan")));
                cSimpanan.moveToFirst();
                String simpanan = ModulTabungan.getString(cSimpanan,"simpanan") ;
                String anggota = ModulTabungan.getString(cSimpanan,"namaanggota") ;
                String notransaksi = ModulTabungan.getString(c,"notransaksi") ;
                String tgl = ModulTabungan.dateToNormal(ModulTabungan.getString(c,"tgltransaksi")) ;
                String masuk = ModulTabungan.removeE(ModulTabungan.getString(c,"masuk")) ;
                String keluar = ModulTabungan.removeE(ModulTabungan.getString(c,"keluar")) ;
                String saldo = ModulTabungan.removeE(ModulTabungan.getString(c,"saldo")) ;
                String ket =  "";
                if (ModulTabungan.getString(c,"kode").equals("T")){
                    ket=Double.valueOf(ModulTabungan.parseDF(masuk))>0?
                            getText(R.string.cetak_line5m).toString():
                            getText(R.string.cetak_line5k).toString();
                }else {
                    ket=getText(R.string.listtransaksiproses_tv3b).toString();
                }

                addLabel(sheet,col++, row, ModulTabungan.intToStr(no));
                addLabel(sheet,col++, row, simpanan);
                addLabel(sheet,col++, row, anggota);
                addLabel(sheet,col++, row, notransaksi);

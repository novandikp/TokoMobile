...
public class MenuExportExcelJenisTabungan extends AppCompatActivity {
    String tab;
    View v;
    DatabaseTabungan db;
    String path;
...
//        ModulTabungan.btnBack("Export Excel",getSupportActionBar());
    ...
    public void export(View view) throws IOException, WriteException {
        ...
        nama = getText(R.string.laporan_jenissimpanan).toString();
        exJenis();

    }

    private void exJenis() throws IOException, WriteException{
        ...
        Cursor c = db.sq(QueryTabungan.select("tbljenissimpanan")) ;
        if(c.getCount() > 0){
            int no = 1 ;
            while(c.moveToNext()){
                int col = 0 ;

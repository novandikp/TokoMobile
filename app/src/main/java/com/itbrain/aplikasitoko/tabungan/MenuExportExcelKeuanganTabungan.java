...
public class MenuExportExcelKeuanganTabungan extends AppCompatActivity {
    String tab;
    View v;
    DatabaseTabungan db;
    String path;
...
//        ModulTabungan.btnBack("Export Excel",getSupportActionBar());
    ...
    public void export(View view) throws IOException, WriteException {
        ...
        nama = getText(R.string.laporan_keuangan).toString();
        exKeuangan();
    }
    ...
    private  void exKeuangan() throws IOException, WriteException {
        ...
        if(c.getCount() > 0){
            ...
            while(c.moveToNext()){
                int col = 0 ;
                String jenis = "";
                if (!ModulTabungan.getString(c,"masuk").equals("0")||ModulTabungan.getString(c,"keluar").equals("0")){
                    jenis="Pemasukan";
                }else {
                    jenis="Pengeluaran";
                }
                String tanggal = ModulTabungan.getString(c,"tgltransaksi");
                String faktur = ModulTabungan.getString(c,"faktur");
                String keterangan= ModulTabungan.getString(c,"keterangantransaksi");
                String masuk = ModulTabungan.getString(c,"masuk");
                String keluar = ModulTabungan.getString(c,"keluar");
                String saldo = ModulTabungan.getString(c,"saldo");
